...
fn setup_connection_and_window(
    window_width: u16,
) -> xcb::Result<(Connection, x::Window, ScreenInfo)> {
    ...
    let cookie = conn.send_request_checked(&x::CreateWindow {
        ...
        visual: screen_info.root_visual,
        value_list: &[
            x::Cw::BackPixel(screen_info.white_pixel),
...
fn main() -> xcb::Result<()> {
    ...
    let (conn, window, screen) = setup_connection_and_window(WINDOW_WIDTH)?;
